...
def test_factorial_alignment_persists_all_seeds_draws_and_selection(monkeypatch) -> None:
    ...
    params = {
        "protocol_version": "e02b-test",
        "protocol_path": "studies/test/e02b.md",
        "training_rows": 12,
        "device": "cpu",
    ...
    assert len(rankings) == 3600
    assert len(query_metrics) == 288
    assert len(summaries) == 432
    ...
    report.update(
        {
            "protocol": params["protocol_path"],
            "compute_authorization": expected_compute,
            "alignment_elapsed_seconds": 10.0,
            "alignment_estimated_compute_cost_usd": 10.0 / 3600.0,
            "runtime": {
                "python": "3.13.0",
                "packages": {
                    "numpy": "test",
            ...
            "git": {
                ...
                "worktree_status_sha256": "e3b0c44298fc1c149afbf4c8996fb924"
    ...
    def validate_version(
        *,
        ...
        metric_table: pd.DataFrame = query_metrics,
        draw_table: pd.DataFrame = draws,
        version_report: dict[str, Any] = report,
    ) -> dict[str, Any]:
        return validate_alignment_outputs(
            pairs,
            queries,
            states,
            input_manifest,
    ...
    assert validation["status"] == "passed_e02b_alignment_readback"

    changed_compute_report = {
        **report,
        "compute_authorization": {**expected_compute, "instance_hour_limit": 2.0},
    }
    with pytest.raises(ValueError, match="compute authorization changed"):
        validate_version(version_report=changed_compute_report)

    dirty_report = {**report, "git": {**report["git"], "worktree_dirty": True}}
    with pytest.raises(ValueError, match="clean Git commit"):
        validate_version(version_report=dirty_report)

    changed_rankings = rankings.copy()
    original_state = str(changed_rankings.loc[0, "state"])
    changed_rankings.loc[0, "state"] = (
        "verified" if original_state != "verified" else "contradicted"
    )
    changed_report = {**report, "output_hashes": dict(report["output_hashes"])}
    changed_report["output_hashes"]["query_rankings_sha256"] = dataframe_content_sha256(
        changed_rankings,
        sort_columns=OUTPUT_SORT_COLUMNS["query_rankings_sha256"],
    )
    with pytest.raises(ValueError, match="ranking state differs"):
        validate_version(ranking_table=changed_rankings, version_report=changed_report)

    invented_scores = rankings.copy()
    invented_scores.loc[0, "score"] = float(invented_scores.loc[0, "score"]) + 1.0
    score_report = {**report, "output_hashes": dict(report["output_hashes"])}
    score_report["output_hashes"]["query_rankings_sha256"] = dataframe_content_sha256(
        invented_scores,
        sort_columns=OUTPUT_SORT_COLUMNS["query_rankings_sha256"],
    )
    with pytest.raises(ValueError, match="query rankings differ from accepted features"):
        validate_version(ranking_table=invented_scores, version_report=score_report)

    invented_paired = paired.copy()
    invented_paired.loc[0, "sequence_to_description_r1"] = 0.123
    paired_report = {**report, "output_hashes": dict(report["output_hashes"])}
    paired_report["output_hashes"]["paired_metrics_sha256"] = dataframe_content_sha256(
        invented_paired,
        sort_columns=OUTPUT_SORT_COLUMNS["paired_metrics_sha256"],
    )
    with pytest.raises(ValueError, match="paired metrics differ from accepted features"):
        validate_version(paired_table=invented_paired, version_report=paired_report)

    invented_draws = draws.copy()
    invented_draws["utility"] = 0.75
    draw_report = {**report, "output_hashes": dict(report["output_hashes"])}
    draw_report["output_hashes"]["bootstrap_draws_sha256"] = dataframe_content_sha256(
        invented_draws,
        sort_columns=OUTPUT_SORT_COLUMNS["bootstrap_draws_sha256"],
    )
    with pytest.raises(ValueError, match="bootstrap draws differ from accepted features"):
        validate_version(draw_table=invented_draws, version_report=draw_report)

    changed_metric_identity = query_metrics.copy()
    changed_metric_identity.loc[0, "semantic_query_id"] = "corrupted"
    metric_report = {**report, "output_hashes": dict(report["output_hashes"])}
    metric_report["output_hashes"]["query_metrics_sha256"] = dataframe_content_sha256(
        changed_metric_identity,
        sort_columns=OUTPUT_SORT_COLUMNS["query_metrics_sha256"],
    )
    with pytest.raises(ValueError, match="query metrics identities differ"):
        validate_version(
            metric_table=changed_metric_identity,
            version_report=metric_report,
        )
